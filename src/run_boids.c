...
s32 run_boids(void)
{
	...
	BoidGPUSimulation boid_sim = create_boid_gpu_simulation(main_arena, device, MiB(8));
	if(0){
	...
	while(true)
	{
		...
		ui_context_next_frame(ui_context, frame_arena, fixed_camera);
		if(sample_count)
			ui_context->pixel_granularity = 1.0f / (f32)(most_significant_bit_position(sample_count) + 1);
		{
			ui_test();
			poll_ui(pe);
		}
...

		{
			...
			{
				f32x2 p;
				f32 r = fabs(romu_quad_f32(&rq)) * 0.05 + 0.05;
				u32 try_count = 0;
				do{
					try_count++;
				...
				if(p.y - r > bottom)
					continue;
				draw_circle(vb, perfect_circle(camera, r), p, r, f32x2_set1(0.0), 0.0, f32x4_set(1.0, 0.0, 0.0, 1.0), f32x4_set(0.5, 0.0, 0.0, 0.0));
			}

			{
				f32x2 p[] = {{-0.5, -0.5},{0.5, 0.5}};
				f32 r = 0.1;
				if(pe.a.pressed)
					r = 0.0f;
					u32 q = 10;
					draw_rounded_rectangle_outline(vb, q, r, 0.1, p, 0, f32x4_set(1.0, 0.0, 1.0, 0.1));
					draw_rounded_rectangle_outline(vb, q, r, 0.001, p, 0, f32x4_set(1.0, 1.0, 1.0, 0.9));
					draw_rounded_rectangle(vb, q, r, p, 0, f32x4_set(0.0, 0.0, 1.0, 0.1));
			}
		...
		wait_and_reset_graphics_fence(render_fences[frame_index]);
		{
			...
			u64 draw_time = 0;
			{
				...
				{
					str = str8_print(scratch.arena, "Boids:\n Max = %u64\n Count = %u64\n", boid_sim.max_boid_count, boid_sim.boid_count);
					pen = draw_str8_wrap(vb, fixed_camera, pen, window->size.x, str, text_pt, f32x4_color_white);
				}
	...
	destroy_boid_gpu_simulation(boid_sim);
