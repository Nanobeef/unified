...
INFO_FOUND:
	free(reply);
	 ...
	 u32x2 chosen_monitor_position = {0,0};
	u32x2 chosen_monitor_size = {0,0};

	{
		xcb_randr_get_monitors_cookie_t cookie = xcb_randr_get_monitors(connection, screen->root, 1);
		xcb_randr_get_monitors_reply_t *reply = xcb_randr_get_monitors_reply(connection, cookie, NULL);
		...
		{
			xcb_randr_monitor_info_t * m = iter.data;
			u32 area = m->width * m->height;
			if(area > largest_area)
			{
				largest_area = area;
				chosen_monitor_position = u32x2_set(m->x, m->y);
				chosen_monitor_size = u32x2_set(m->width, m->height);
			}
		}
		free(reply);
	}
	...
	xcb_create_window(
		...
		handle,
		screen->root,
		chosen_monitor_position.x, chosen_monitor_position.y,
		size.x, size.y,
		1,
